...
def split_lines(text: str) -> str:
    ...
    text = re.sub(rf'(?<=. |\d\d)({OCC_HOUSE_GROUP}|{HONORS_GROUP}) (?=[A-Z][a-zä]+\s*[,.] [A-Z][a-zä]+,? )', '\\1\n', text)
    text = re.sub(rf'(?<![EWNS] |.\d)((?: |\d\d){SCHOOL_DEG_GROUP}) (?=[A-Z][a-zä]+\s*[,.] [A-Z][a-zä]+,? )', '\\1\n', text)
...
def split_lines2(text: str) -> str:
    ...
    text = re.sub(r'(?<=[^\d]\d\d|\d\d\)) (?=\S+\s*[,.] [A-Z][a-zä]+,? )', '\n', text)
    return text
...
zip_re = re.compile(r'^(.*?(?:[A-Z][a-z]{1,4}|[A-Z]\.? ?[A-Z])(?:,|\.)?,?\s*\d{5}(?:-\d{4})?) ?(.*)')
zip2_re = re.compile(rf'^(.*?(?:[A-Z][a-z]{{1,4}}|[A-Z]\.? ?[A-Z]))[,.]?,? ((?:{OCC_HOUSE_GROUP}[,.]?\s|{SCHOOL_DEG_GROUP}[\s\d]).*)')
...
profile_re = re.compile(r'^((?:[A-Z][A-Za-z\'ä]*[ \-]|del? )*[A-Z][A-Za-z\'ä]*)[,.]\s*(.+?[,.\)]) ?(.*)')
